...
export default function ClipItemCard({ itemId, children }: ClipItemCardProps) {
  ...
  const [brand, setBrand] = useState<ItemBrandData | null>(null)
  const [calc, setCalc] = useState<ItemCalcData | null>(null)
  const [isCliped, setIsCliped] = useState(true)
  useEffect(() => {
    ;(async () => {
      const thumbnailData = await getItemThumbnail(itemId)
      setThumbnail(thumbnailData)
      const basicInfoData = await getItemBasicInfo(itemId)
      setBasicInfo(basicInfoData)
      const brandData = await getItemBrand(itemId)
      setBrand(brandData)
      const calcData = await getItemCalc(itemId)
      setCalc(calcData)
      const clipStatus = await getClip(itemId)
      setIsCliped(clipStatus)
    })()
